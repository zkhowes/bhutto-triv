...
> Last updated: 2026-05-25 (auto-close/auto-skip warning now fires ~1h before deadline)
...
### Done
- [x] Auto-close / auto-skip warning fires ~1h before deadline (was firing AT close) — *The "round is about to close / you're about to be skipped" warning was effectively coinciding with the close instead of leading it by ~1h. Root cause: the cron (`src/app/api/cron/notifications/route.ts`) only *looked at* rounds already `updatedAt + 24h` stale (§3 auto-skip, §3b auto-close), but the close deadline (`deferredSkipDeadline` = `updatedAt + 24h`) is the same threshold — so a round became warn-eligible and close-eligible on the same 15-min tick; the warn branch was only reachable inside the thin quiet-hours-deferral gap. Fix: widened §3/§3b query eligibility to `twentyThreeHoursAgo` (rounds enter the sweep ~1h early) and replaced the two-way branch with three-way — `now >= deadline` → close/skip; `deadline - now <= WARN_LEAD_MS (1h)` → warn; else too-early-skip-tick. Warning still fires once per player/round (existing notification-row dedup: cron-level guard keyed `roundId+type` for auto_close, `roundId+userId+type` in the helper). Quiet-hours deferral preserved — the 1h check measures against the deferred deadline. §2 action reminders left on their strict 24h trigger. Separately noted but left as-is: §1 (`notifyAboutToBeSkipped`, the originally-intended 1h warning) is dead code — it queries `deadlineAt`, a schema column no code ever writes, so it always returns zero rows; redundant now with §3b. 193/193 tests pass, typecheck + build clean.*
- [x] Pre-ice cleanup pass — *Before parking the project for a stretch. (1) `npm audit fix` resolved 6/11 vulnerabilities (lockfile-only — no package.json edits, no breaking changes); remaining 5 (1 critical flatted DoS, 3 high vite/glob/minimatch) all gated behind a Next 14 → 16 upgrade and deferred. (2) Deleted `src/components/question/WorkshopEmbed.tsx` — 519-line dead component with zero imports, leftover from an earlier workshop design iteration. (3) Removed unused `notifyAllAnswersIn` export from `src/lib/notifications.ts` (no call sites). (4) Pinned `@anthropic-ai/sdk` from `^0.32.1` to exact `0.32.1` — 0.x ranges silently drift on minor bumps, dangerous for an idle project. (5) Deduped `AutoSkipCountdown` and `DashboardSkipCountdown`: identical `formatRemaining`/`computeDeadline` + 3h-amber / 1h-red-pulse threshold logic extracted into a shared `src/components/game/skipCountdown.ts` (`useSkipCountdown` hook + `formatSkipRemaining` + `skipColorClass` helpers). Both components shrunk from ~70 lines to ~25; rendered output byte-stable. Audit's "stale Price is Right copy" claim was a false positive — only the internal `price_is_right` format key and `isPriceIsRight` variable remain, both intentional per CLAUDE.md (no DB migration). 193/193 tests pass, build green; net -638 lines.*
