...
export default function AutoSkipAnnouncementModal({ leagueId }: AutoSkipAnnouncementModalProps) {
  ...
  useEffect(() => {
    // Check for unread auto_skip_enabled notification for this league
    fetch("/api/notifications/list")
      .then((r) => r.json())
      .then((notifications: Array<{ id: string; type: string; leagueId: string | null; isRead: boolean }>) => {
        const unread = notifications.find(
          (n) => n.type === "auto_skip_enabled" && n.leagueId === leagueId && !n.isRead
        );
        if (unread) {
